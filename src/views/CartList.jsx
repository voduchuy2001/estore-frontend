      ...
      <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
        <div className="rounded-lg md:w-2/3">
          {carts.map((cart) => (
            <div
              key={cart._id}
              className="mb-6 justify-between rounded-lg bg-white p-6 shadow-sm sm:flex sm:justify-start"
            >
              <img
                src={`http://localhost:8000/images/${cart.image}`}
                alt="product-image"
        ...
        {carts.length > 0 && (
          <div className="my-6 h-full rounded-md border bg-white p-6 shadow-sm md:mt-0 md:w-1/3">
            ...
            <Link
              to={'/checkout'}
              className="mt-2 inline-flex w-full items-center justify-center gap-x-2 rounded-lg border border-transparent bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:pointer-events-none disabled:opacity-50 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
            >
              Check out
            </Link>
          </div>
